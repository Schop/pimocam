...
<body>
    ...
    <div class="container">
        ...
        <?php endif; ?>
        <p class="text-muted">
            Showing <?= count($pageFiles) ?> of <?= $total ?> clips
            (page <?= $page ?> of <?= $totalPages ?>)
        </p>

        <form method="post" action="delete.php" id="deleteForm">
            <input type="hidden" name="type" value="clips">
            ...
            <div class="d-flex align-items-center gap-3 mb-3">
                ...
                <button type="submit" name="action" value="delete_all" class="btn btn-outline-danger btn-sm" onclick="return confirm('Delete ALL <?= $total ?> clips? This cannot be undone.')">Delete All (<?= $total ?>)</button>
            </div>
    ...
    <script>
        ...
        function confirmDeleteSelected() {
            ...
            if (checked === 0) {
                alert('Select at least one clip to delete.');
                return false;
            }
            return confirm('Delete ' + checked + ' selected clip(s)? This cannot be undone.');
        }
    </script>
</body>
</html>
